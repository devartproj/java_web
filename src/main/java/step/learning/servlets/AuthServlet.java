...
public class AuthServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if( login.isEmpty() || password.isEmpty() ) {
            sendResponse( resp, 400, "login and password could not be empty" ) ;
            return;
        }
    ...
    private void sendResponse( HttpServletResponse resp, int statusCode, Object body ) throws IOException {
...
Задачі мають декілька способів вирішення
- через механізм серверних сесій
- за допомогою токенів

Токен (дослівно - жетон, перепустка) - якісь дані, що використовуються для
авторизації - для одержання доступу до ресурсів обмеженого типу.
Існують пропозиції стандартів, типу JWT
 */